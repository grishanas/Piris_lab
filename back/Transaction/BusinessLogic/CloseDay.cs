...
namespace lab.Transaction.BusinessLogic
{
    public class CloseDay
    {
        ...
        private enum FindBalanceCredit
        {
            d2470 = 2470
        }
        ...
        private async Task<Balance> BalanceCredit(Account account)
        {
        ...
        private bool IsCreditBalance(Account account)
        {
            foreach (int i in Enum.GetValues(typeof(FindBalanceCredit)))
            {
                if (account.account_code == i.ToString())
                    return true;
            }
            ...
            for (int i= 0; i< acs.Count;i++)
            {
                try
                {
                    ...
                    } else if (IsCredit(acs[i]))
                    {
                        ...
                        try
                        {
                            await _balanceContext.AddBalance(bal);
